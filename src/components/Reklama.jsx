...
export default class Reklama extends Component {
  constructor(props) {
    super(props);
    this.state = {
      products: [], 
  ...
  render() {
    ...
    return (
      <div className='container py-[140px] bg-[#14110E]'>
        <ul className='flex items-center gap-[36px] justify-center mb-[140px]'>
          {limitedProducts.map(product => (
            <li key={product.id}>
              <div className='flex items-center gap-[12px]'>
                <span className='font-[400] text-[24px] leading-[29px] text-[#C99E71]'>0{product.id}</span>
                <h2 className='font-[400] text-[24px] leading-[29px] text-white'>{product.title}</h2>
              </div>
              <img
                src={product.image}
                alt={product.title}
